<script lang="ts">
  ...
  `;
    ...
    void main() {
      ...
      float r = texture2D(uTexture, uv_hit + flow + vec2(dispersion, 0.0)).r;
      float g = texture2D(uTexture, uv_hit + flow).g;
      float b = texture2D(uTexture, uv_hit + flow - vec2(dispersion, 0.0)).b;
