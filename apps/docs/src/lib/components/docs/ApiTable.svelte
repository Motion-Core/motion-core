						...
						>
							{#if key === "type" && typeof row[key] === "string" && row[key].includes("|")}
								{#each row[key].split("|") as type, i (type + i)}
									<code
										class={cn(
											"card-highlight relative rounded-lg bg-card-muted px-1.5 py-0.5 font-normal whitespace-nowrap text-foreground shadow-sm mono",
										)}
									>
										{type.trim()}
									</code>
							...
							{:else}
								<code
									class={cn(
										"card-highlight relative rounded-lg bg-card-muted px-1.5 py-0.5 font-normal text-foreground shadow-sm mono",
									)}
								>
									{row[key]}
								</code>
