...
const getThemeButtonClasses = (mode: ThemeMode) => {
	const base =
		"inline-flex size-6 items-center justify-center rounded-md border text-[10px] transition-colors duration-150 ease-out focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring/40";
...
<main class="min-h-svh w-full bg-background p-4">
	<div class="mx-auto grid h-full gap-6 lg:grid-cols-4">
		<aside
			class="flex flex-col justify-between gap-6 lg:sticky lg:top-4 lg:col-span-1 lg:self-start"
		>
			<header class="flex items-center gap-2">
				<span class="text-xl font-medium text-foreground">Motion Core</span>
			</header>
			<div class="space-y-4">
				<p class="text-xs leading-relaxed text-foreground/70 text-balance">
					Motion Core is your playground of GSAP primitives and Three.js canvases ready to drop into Svelte
					experiences. Compose storytelling backdrops, kinetic typography, and interactive UI states without
					starting from scratch.
				</p>
				<nav class="flex gap-2 text-xs font-medium uppercase tracking-wide">
		...
		<section class="lg:col-span-3 lg:max-h-[calc(100svh-2rem)] lg:overflow-y-auto lg:pr-2">
			<div class="columns-1 gap-4 space-y-4 sm:columns-2 lg:columns-3 [column-fill:balance] pb-6">
				{#each mockComponents as component}
					<article class="break-inside-avoid overflow-hidden rounded-xl border border-border bg-card p-1 flex flex-col items-center justify-center gap-2">
						<div class="relative h-56 w-full bg-muted">
							<img
								loading="lazy"
						...
						<button
							class="w-full px-4 py-3 text-xs font-medium uppercase tracking-wide text-foreground rounded-lg bg-card-muted"
						>
							{component.name}
						</button>
					</article>
