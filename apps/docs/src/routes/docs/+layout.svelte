...
<main class="relative h-dvh bg-card text-foreground lg:py-2">
	...
	<div
		class="input-highlight relative mx-auto w-full max-w-4xl overflow-hidden bg-background pt-12 md:overflow-visible lg:ml-88 lg:max-h-[calc(100dvh-1rem)] lg:rounded-2xl lg:pt-0 xl:mr-88"
	>
		<ScrollArea
								...
								<div class="flex flex-wrap gap-2">
									{#each dependencies as dep (dep)}
										<a
											...
											target="_blank"
											rel="noreferrer"
											class="card-highlight relative inline-flex items-center rounded-full bg-card px-2.5 py-0.5 text-xs font-medium text-foreground/70 shadow-sm transition-[color] duration-150 ease-out hover:text-foreground"
										>
											{dep}
										</a>
									{/each}
